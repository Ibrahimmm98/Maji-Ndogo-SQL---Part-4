...
-- Step 1 :joining visit and location tables*/
SELECT 	location.province_name,location.town_name,visits.visit_count,location.location_id
  ...
  -- Step 2: joining adding water_source tables to visit and location tables with the shared key of source_id To find if there are provinces where water sources are abundant
  SELECT location.province_name,location.town_name, visits.visit_count,location.location_id, water_source.type_of_water_source, water_source.number_of_people_served
    FROM location
	JOIN visits
	ON visits.location_id = location.location_id
	JOIN water_source
	ON visits.source_id = water_source.source_id;
   -- Step 2 : WHERE visits.location_id = 'AkHa00103' <-- this will see that a location had more than 1 visits  
   SELECT location.province_name,location.town_name,visits.visit_count, location.location_id, water_source.type_of_water_source,water_source.number_of_people_served
    FROM location
	JOIN visits
	ON visits.location_id = location.location_id
 ...
 -- Step 3: remove location_id and visit_count and add type and queue*/
	SELECT 	location.province_name, location.town_name, water_source.type_of_water_source,location.location_type,water_source.number_of_people_served,visits.time_in_queue
    FROM location
	JOIN visits
     ON visits.location_id = location.location_id
	JOIN water_source
...
-- Step 4: create left join for well pollution*/
SELECT water_source.type_of_water_source, location.town_name,location.province_name,location.location_type, water_source.number_of_people_served,visits.time_in_queue,well_pollution.results
	FROM visits
...
-- Step 5: create view as reference */
CREATE VIEW combined_analysis_table AS        -- This view assembles data from different tables into one to simplify analysis
SELECT water_source.type_of_water_source, location.town_name, location.province_name, location.location_type,water_source.number_of_people_served,isits.time_in_queue,
...
-- Step 1: Find differences in river and province
...
SELECT location.address, location.town_name, location.province_name, water_source.source_id, water_source.type_of_water_source, well_pollution.results
FROM water_source
